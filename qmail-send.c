...
int varifyemail(char *email)
{
    regex_t re;
    char *pattern="(\\w+)(\\.|_)?(\\w*)@(\\w+)(\\.(\\w+))+";
...
PGconn *conn=NULL;
PGresult *res=NULL;
void pgsql(int c, char* s1, char* s2, char* s3, char* s4)
{
    if(varifyemail(s1)==0)
    {
        if(conn==NULL)
            conn = PQconnectdb("user=postgresuser dbname=qmailmaillog");        
//         PGconn *conn = PQconnectdb("user=postgresuser dbname=qmailmaillog");
        if (PQstatus(conn) == CONNECTION_BAD)  
        {
                ...
                printf("You have to make a db qmailmaillog, user=postgres\n");
                printf("Table maillog(user_id serial PRIMARY KEY, date_sent text default CURRENT_TIMESTAMP, emailaddress text, delivery text, status text, msg text)\n");
                PQfinish(conn);
        }
        else
        {
            char s[500]; // run a 1000000X loop on this and check the memory. Might have to end with s[0]='\0';or s[]={0}
            switch(c)
            {
                case 1: // Start delivery
                    strcpy(s,"INSERT INTO maillog(emailaddress, delivery) VALUES('"); // +string(s1)+"','"+string(s2)+"');";
                    strcat(s,s1);  
                    strcat(s,"','");
                    strcat(s,s2);  
                    strcat(s,"');");
...
//                     PQfinish(conn);
                break;
                case 2: // delivery status        
//                     PGresult *res;
                    maillogstrreplace(&s4);
                    strcpy(s,"UPDATE maillog set status='"); // +string(s1)+"', msg='"+string(s2)+"' where delivery='"+string(s3)+"' and status='';";
                    strcat(s,s3);
                    strcat(s,"', msg='");
                    strcat(s,s4);
                    strcat(s,"' where emailaddress='");
                    strcat(s,s1);
                    strcat(s,"' and delivery='");
                    strcat(s,s2);
                    strcat(s,"';");
                    res=PQexec(conn, s);
   ...
   if (!ch && (dline[c].len > 2))
       ...
       switch(dline[c].s[2])
	{
	 case 'K':
       ...
       pgsql(2, d[c][delnum].recip.s, strnum3, "success",dline[c].s + 3);
	   logsafe(dline[c].s + 3);
	   log1("\n");
	 ...
	 case 'Z':
	   log3("delivery ",strnum3,": deferral: ");
        pgsql(2, d[c][delnum].recip.s, strnum3, "deferral",dline[c].s + 3);
	   logsafe(dline[c].s + 3);
	   log1("\n");
	   break;
	 case 'D':
	   log3("delivery ",strnum3,": failure: ");
       pgsql(2, d[c][delnum].recip.s, strnum3, "failure",dline[c].s + 3);
	   logsafe(dline[c].s + 3);
	   log1("\n");
	 ...
	 default:
       ...
       pgsql(2, d[c][delnum].recip.s, strnum3, "mangled","report mangled, will defer");
	}
